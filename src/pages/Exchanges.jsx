...
const Exchanges = () => {
    ...
    const fetchUserStocks = async () => {
        if (!isAuthenticated) {
            console.log('Usuario no autenticado, saltando fetchUserStocks');
            return;
        }
    ...
    const fetchExchangeHistory = async () => {
        if (!isAuthenticated) {
            console.log('Usuario no autenticado, saltando fetchExchangeHistory');
            return;
        }

        try {
            const response = await makeAuthenticatedRequest('http://localhost:3000/admin/exchange-history');
            if (response.ok) {
                const data = await response.json();
    ...
    const loadExternalOffers = async () => {
        if (!isAuthenticated) {
            console.log('Usuario no autenticado, saltando loadExternalOffers');
            return;
        }

        try {
            const response = await makeAuthenticatedRequest('http://localhost:3000/admin/external-offers');
            const data = await response.json();
    ...
    const handleCreateOffer = async () => {
        ...
        setLoading(true);
        try {
            const response = await makeAuthenticatedRequest('http://localhost:3000/admin/auctions/offer', {
                method: 'POST',
                body: JSON.stringify({
                    symbol: symbol,
    ...
    const handleCreateProposal = async (originalOffer) => {
        ...
        setLoading(true);
        try {
            const response = await makeAuthenticatedRequest('http://localhost:3000/admin/auctions/proposal', {
                method: 'POST',
                body: JSON.stringify({
        ...
        return (
            <div className="exchanges-container">
                ...
                <div className="message error">
                    ⚠️ Necesitas estar autenticado para acceder a esta funcionalidad.
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="exchanges-container">
            ...
            <div className="admin-notice">
                🔐 <strong>Funcionalidad de Administrador:</strong> Solo usuarios administradores pueden realizar intercambios.
            </div>
